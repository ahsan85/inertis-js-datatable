...
<template>
    <div class="flex items-center">
        <div class="flex items-center my-4 w-1/5">
            ...
            <select v-model="pagination.per_page" @change="onPerPageChange($event)"
        ...
        <div v-if="links.length > 3" class="flex justify-end w-full py-4 space-x-2">
            <div v-for="(link, k) in links" :key="k">
                ...
                <Link v-else class="px-4  "
                    :class="{ ' py-2 items-center   border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest  focus:outline-none  transition ease-in-out duration-150 focus:border-indigo-900 focus:shadow-outline-indigo hover:bg-indigo-800 active:bg-indigo-900 bg-indigo-900': link.active }" :href="link.url" v-html="link.label" />
            </div>
        </div>
    </div>
</template>